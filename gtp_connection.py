...
class GtpConnection():
    ...
    def policy_moves_cmd(self, args):
        ...
        """
        prob_moves= GoBoardUtil.prob_moves(self.board)
        finallist=[]
        #print(prob_moves)
        if prob_moves != {}:
            response={}
            for item in prob_moves:
                response[item]="{:.5f}".format(prob_moves[item])
        #sorted(response.values(),reverse=True)
            
            s = [(k, response[k]) for k in sorted(response, key=response.get, reverse=True)]
            for k, v in s:
                finallist.append(k)
                finallist.append(v)
            self.respond(' '.join(finallist))
        else:
            finallist.append("pass")
            finallist.append("1.00000")
            self.respond(' '.join(finallist))
